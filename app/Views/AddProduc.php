...
<?= $this->section('content') ?>
    ...
    <?php if (session()->getFlashdata('success')): ?>
        <div class="mb-4 bg-green-100 border border-green-400 text-green-700 px-4 py-2 sm:px-6 sm:py-3 rounded text-sm sm:text-base">
             <?= session()->getFlashdata('success') ?>
        </div>
    <?php elseif (session()->getFlashdata('error')): ?>
        <div class="mb-4 bg-red-100 border border-red-400 text-red-700 px-4 py-2 sm:px-6 sm:py-3 rounded text-sm sm:text-base">
    ...
    <script>
        ...
        function validarFormulario() {
            ...
            const categoria = document.querySelector('[name="categoria"]').value; // Para select, no trim()
            const subcategoria = document.querySelector('[name="subcategoria"]').value; // Para select, no trim()
            const imagenes = document.querySelector('[name="imagen[]"]').files;

            // Validación de campos de texto y selects
            if (!nombre || !marca || !cantidad || !precio || !categoria || !subcategoria) {
                mostrarMensaje("Por favor, completa todos los campos obligatorios.", 'error');
                return false;
            }

            // Validación de cantidad y precio (asegurar que sean números válidos y positivos)
        ...
        function previewImagenes(event) {
            ...
            // Si no se seleccionan archivos, ocultar el div de previsualización o limpiarlo
            ...
            Array.from(event.target.files).forEach(file => {
                ...
                reader.onload = e => {
                    ...
                    // Ajustado el tamaño de la imagen de previsualización para ser responsivo
                    img.className = 'w-full h-auto object-cover rounded aspect-square'; // aspect-square para mantener proporción
